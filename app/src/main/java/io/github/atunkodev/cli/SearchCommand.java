...
public class SearchCommand implements Runnable {
    ...
    @Override
    @Requirements({"CLI_0004"})
    public void run() {
        PrintWriter out = spec.commandLine().getOut();
        Set<RecipeField> searchFields = (fields != null && !fields.isEmpty()) ? fields : Set.of(RecipeField.values());
        List<RecipeInfo> recipes = sorted(service.search(query, searchFields));
        render(out, recipes);
        out.flush();
    }
