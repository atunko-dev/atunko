...
- [x] 1.1 Add requirements CORE_0020 (custom recipe YAML loading) with children
...
- [x] 1.2 Add SVCs SVC_CORE_0020(.1/.2/.3), SVC_CORE_0021(.1), SVC_CORE_0022(.1),
      SVC_TUI_0007(.1), SVC_TUI_0008 to
      `docs/reqstool/software_verification_cases.yml`

## 2. Core: custom recipe YAML loading

- [x] 2.1 Add `ConfigDirs` (XDG config dir resolution: `atunko.config.dir`
      property override, `$XDG_CONFIG_HOME/atunko`, `~/.config/atunko`) and
      `UserRecipeFiles.discover(dir)` listing `*.yml`/`*.yaml` recipe files
- [x] 2.2 `EnvironmentProvider`: accept user recipe YAML files, load each via
      ...
      `loadWarnings()`
- [x] 2.3 Add `@Requirements({"atunko:CORE_0020..."})` on the implementing code
- [x] 2.4 Tests: YAML recipe discovered and classified USER, auto-discovery
      listing, config dir override, malformed YAML skipped with a warning while
      the valid file still loads (@TempDir fixtures)
- [x] 2.5 Add `@SVCs({"atunko:SVC_CORE_0020..."})` on the tests from 2.4

## 3. Core: favorites and recently used

- [x] 3.1 Add `FavoritesService` (toggle/isFavorite/favorites, persisted to
      `favorites.yml`, missing/malformed file reads as empty) and
      `FavoritesFilter { ALL, FAVORITES }` with `matches`/`next`
- [x] 3.2 Add `RecentRecipesService` (`record(names)` newest-first with ISO-8601
      timestamps, dedupe, cap 20, persisted to `recent.yml`) and
      `SortOrder.RECENT` with a `comparator(recentFirst)` overload
- [x] 3.3 Add `@Requirements({"atunko:CORE_0021..."})` and
      `@Requirements({"atunko:CORE_0022..."})` on the implementing code
- [x] 3.4 Tests: favorite toggle persistence and reload, malformed favorites
      file resilience, filter cycling; recent recording order/dedupe/cap and
      persistence, recent-first comparator (@TempDir files)
- [x] 3.5 Add `@SVCs({"atunko:SVC_CORE_0021..."})` and
      `@SVCs({"atunko:SVC_CORE_0022..."})` on the tests from 3.4

## 4. CLI

- [x] 4.1 `ListCommand` and `SearchCommand`: repeatable `--recipes-file <path>`,
      auto-discovered config-dir recipe files added to the discovery
      environment, load warnings printed to stderr
- [x] 4.2 Add `@Requirements({"atunko:CORE_0020.1", "atunko:CORE_0020.2",
      "atunko:CORE_0020.3"})` on the implementing code
- [x] 4.3 Tests: `--recipes-file` with a fixture YAML lists the user recipe under
      `--source user`; a malformed file warns on stderr and the command still
      succeeds
- [x] 4.4 Add `@SVCs({"atunko:SVC_CORE_0020.1", "atunko:SVC_CORE_0020.3"})` on
      the tests from 4.3

## 5. TUI

- [x] 5.1 `TuiCommand`: `--recipes-file` option and warning pass-through;
...
- [x] 5.2 `RecipeListRenderer`: ` *` favorite marker; `BrowserView`: `f`/`F`
      keys, third sort tab, `fav:<filter>` status indicator; `HelpOverlay`:
      document `f`, `F`, and the new sort cycle
- [x] 5.3 Add `@Requirements({"atunko:TUI_0007", "atunko:TUI_0007.1",
      "atunko:TUI_0008"})` on the implementing code
- [x] 5.4 Tests: `TuiControllerTest` favorites toggle/filter/persistence and
...
- [x] 5.5 Add `@SVCs({"atunko:SVC_TUI_0007", "atunko:SVC_TUI_0007.1",
