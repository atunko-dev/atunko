...
- [ ] 1.1 Add requirements CORE_0017 (lazy session initialisation), TUI_0005
      (TUI deferred scan + error surfacing), WEB_0004 (Web deferred scan + error
      surfacing) to `docs/reqstool/requirements.yml`
- [ ] 1.2 Add SVCs SVC_CORE_0017(.1/.2), SVC_TUI_0005(.1), SVC_WEB_0004 to
      `docs/reqstool/software_verification_cases.yml`

## 2. Core: lazy SessionHolder

- [ ] 2.1 Tests first: `SessionHolderTest` additions — `initLazy` records dir and
      ...
      unchanged
- [ ] 2.2 Implement `SessionHolder.initLazy(Path)` + `ensureScanned()` per
      design decision 1 (startup still runs `ProjectScannerFactory.detect` for
      fail-fast validation, only `.scan()` deferred)
- [ ] 2.3 Add `@Requirements({"atunko:CORE_0017"})` on the new methods and
      `@SVCs` on the tests from 2.1

## 3. TUI

- [ ] 3.1 `TuiCommand`: replace startup scan with `detect(dir)` validation +
...
- [ ] 3.2 Pilot e2e tests: TUI launches and browses recipes with a project dir
...
- [ ] 3.3 Add `@Requirements({"atunko:TUI_0005"})` on the implementing code and
      `@SVCs` on the tests from 3.2

## 4. Web

- [ ] 4.1 `WebUiCommand`: replace startup scan with `detect(dir)` validation +
...
- [ ] 4.2 Karibu test: view opens with unscanned session; execution triggers
      scan; scan failure surfaces as notification and view stays usable
- [ ] 4.3 Add `@Requirements({"atunko:WEB_0004"})` on the implementing code and
      `@SVCs` on the test from 4.2

## 5. Audit + wrap-up

- [ ] 5.1 Audit all call sites of `SessionHolder.getProjectInfo()` /
      `getProjectEntries()` for pre-scan null/empty tolerance; fix any that
      assume a scanned session
- [ ] 5.2 `./gradlew spotlessApply build` green; `openspec validate --all --strict`
      passes
